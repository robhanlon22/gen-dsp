...
- **AUv3 backend** -- New `auv3` platform (the 15th backend) that generates macOS Audio Unit v3 plugins as App Extensions (`.appex`) inside a host application (`.app`). Uses `cmake -G Xcode` to produce the nested bundle structure required by PluginKit for system-wide AU discovery. Implements `AUAudioUnit` subclass in Objective-C++ with `AUParameterTree` for parameters and `internalRenderBlock` for realtime-safe audio processing. Handles float (gen~) to float (AUv3 non-interleaved) zero-copy I/O. Auto-detects `aufx` (effect) vs `augn` (generator) vs `aumu` (MIDI instrument) from I/O configuration. Requires macOS + Xcode + CMake >= 3.19. Works with both gen~ exports and dsp-graph compiled graphs. Platform key: `"auv3"`.

- **Csound backend** -- New `csound` platform (the 14th backend) that generates Csound opcode plugins via the `csdl.h` C API. Audio inputs map to a-rate args, parameters to k-rate args, audio outputs to a-rate outputs. OENTRY type strings are auto-generated from the gen~ manifest I/O counts. Handles float (gen~) to MYFLT/double (Csound) conversion. Supports sample-accurate timing (`ksmps_offset`/`ksmps_no_end`). Header discovery: macOS CsoundLib64.framework, Homebrew, or `/usr/local/include/csound`. No link against libcsound (header-only plugin API). Works with both gen~ exports and dsp-graph compiled graphs. Platform key: `"csound"`.
