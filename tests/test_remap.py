...
class TestParserInputNames:
    ...
    def test_fm_bells_input_names(self, fm_bells_export: object) -> object:
        """fm_bells should expose two control input names."""
        parser = GenExportParser(fm_bells_export)
        info = parser.parse()
        assert info.input_names == ["carrier", "c/m ratio"]

    def test_gigaverb_input_names(self) -> object:
        """Gigaverb input names should parse without crashing."""
    ...
    def test_no_innames_returns_empty(self) -> object:
        """Exports without innames should still parse cleanly."""
...
class TestApplyInputsAsParams:
    ...
    def test_remap_all_inputs(self, base_manifest: object) -> object:
        """All inputs should be remapped when remap_names is None."""
        ...
        assert result.num_inputs == NUM_0
        # 3 original params + 2 remapped = 5
        assert result.num_params == NUM_5
        assert len(result.remapped_inputs) == NUM_2
        # Check the synthetic param names
        assert result.params[NUM_3].name == "carrier"
        assert result.params[NUM_4].name == "c_m_ratio"  # sanitized

    def test_remap_specific_inputs(self, base_manifest: object) -> object:
        """Only explicitly named inputs should be remapped."""
        input_names = ["carrier", "c/m ratio"]
        ...
        assert result.num_inputs == NUM_1
        # 3 original + 1 remapped = 4
        assert result.num_params == NUM_4
        assert len(result.remapped_inputs) == NUM_1
        assert result.remapped_inputs[NUM_0].input_name == "carrier"
        assert result.remapped_inputs[NUM_0].gen_input_index == NUM_0

    def test_remap_preserves_original_params(self, base_manifest: object) -> object:
        """Original params should be preserved verbatim."""
        input_names = ["carrier", "c/m ratio"]
        result = apply_inputs_as_params(base_manifest, input_names, remap_names=None)
    ...
    def test_remap_gen_input_indices(self, base_manifest: object) -> object:
        """Remapped inputs should preserve their gen input indices."""
        input_names = ["carrier", "c/m ratio"]
        result = apply_inputs_as_params(base_manifest, input_names, remap_names=None)

        assert result.remapped_inputs[NUM_0].gen_input_index == NUM_0
        assert result.remapped_inputs[NUM_1].gen_input_index == NUM_1

    def test_remap_param_indices(self, base_manifest: object) -> object:
        """Remapped inputs should get deterministic param indices."""
        input_names = ["carrier", "c/m ratio"]
        result = apply_inputs_as_params(base_manifest, input_names, remap_names=None)

        assert result.remapped_inputs[NUM_0].param_index == NUM_3
        assert result.remapped_inputs[NUM_1].param_index == NUM_4

    def test_remap_no_input_names_raises(self, base_manifest: object) -> object:
        """Missing input names should raise ValueError."""
        with pytest.raises(ValueError, match="No input names found"):
            apply_inputs_as_params(base_manifest, [], remap_names=None)

    def test_remap_invalid_name_raises(self, base_manifest: object) -> object:
        """Unknown remap names should raise ValueError."""
        input_names = ["carrier", "c/m ratio"]
    ...
    def test_remap_zero_input_manifest_raises(self) -> object:
        """A manifest without inputs should raise ValueError."""
        manifest = Manifest(
        gen_name="gen_exported",
        num_inputs=0,
        num_outputs=2,
...
class TestBuildRemapDefines:
    ...
    def test_no_remaps_returns_empty(self, base_manifest: object) -> object:
        """No remapped inputs -> empty string."""
        assert build_remap_defines(base_manifest) == ""


    def test_with_remaps(self, base_manifest: object) -> object:
    ...
    def test_raw_defs_list(self, base_manifest: object) -> object:
...
class TestBuildRemapDefinesMake:
    ...
    def test_no_remaps_returns_empty(self, base_manifest: object) -> object:
        """No remaps should return an empty string."""
        assert build_remap_defines_make(base_manifest) == ""


    def test_single_flag_var(self, base_manifest: object) -> object:
    ...
    def test_multiple_flag_vars(self, base_manifest: object) -> object:
        ...
        assert len(cflags_lines) > NUM_0
        assert len(cppflags_lines) > NUM_0
        # Same number of defines for each variable
        assert len(cflags_lines) == len(cppflags_lines)


    def test_make_format_no_trailing_newline(self, base_manifest: object) -> object:
...
class TestRemapSerialization:
    ...
    def test_round_trip(self, base_manifest: object) -> object:
        ...
        for orig, rest in zip(
            remapped.remapped_inputs, restored.remapped_inputs, strict=True
        ):
...
class TestRemapProjectGeneration:
    ...
    def _generate_project(
        self, fm_bells_export: object, tmp_path: object, platform: object
    ) -> object:
    ...
    def test_clap_cmake_has_remap_defines(
        self, fm_bells_export: object, tmp_path: object
    ) -> object:
    ...
    def test_vst3_cmake_has_remap_defines(
        self, fm_bells_export: object, tmp_path: object
    ) -> object:
    ...
    def test_au_cmake_has_remap_defines(
        self, fm_bells_export: object, tmp_path: object
    ) -> object:
    ...
    def test_lv2_cmake_has_remap_defines(
        self, fm_bells_export: object, tmp_path: object
    ) -> object:
    ...
    def test_sc_cmake_has_remap_defines(
        self, fm_bells_export: object, tmp_path: object
    ) -> object:
    ...
    def test_chuck_makefile_has_remap_defines(
        self, fm_bells_export: object, tmp_path: object
    ) -> object:
    ...
    def test_vcvrack_makefile_has_remap_defines(
        self, fm_bells_export: object, tmp_path: object
    ) -> object:
