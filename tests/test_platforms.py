...
class TestPlatformRegistry:
    ...
    def test_get_platform_class_pd(self) -> object:
        """Test test get platform class pd."""
        cls = get_platform_class("pd")
        assert cls == PureDataPlatform

    def test_get_platform_class_invalid(self) -> object:
        """Test test get platform class invalid."""
        with pytest.raises(ValueError, match="Unknown platform"):
            get_platform_class("invalid")

    def test_list_platforms(self) -> object:
        """Test test list platforms."""
    ...
    def test_is_valid_platform_pd(self) -> object:
        """Test test is valid platform pd."""
        assert is_valid_platform("pd") is True

    def test_is_valid_platform_max(self) -> object:
        """Test test is valid platform max."""
        assert is_valid_platform("max") is True

    def test_is_valid_platform_invalid(self) -> object:
        """Test test is valid platform invalid."""
...
class TestPlatformBase:
    ...
    def test_platform_has_required_attributes(self) -> object:
        """Test test platform has required attributes."""
    ...
    def test_platform_has_required_methods(self) -> object:
        """Test test platform has required methods."""
        for name in list_platforms():
            platform = get_platform(name)
    ...
    def test_platform_get_build_instructions_returns_list(self) -> object:
        """Test test platform get build instructions returns list."""
        for name in list_platforms():
            platform = get_platform(name)
...
class TestPureDataPlatform:
    ...
    def test_pd_extension_is_platform_specific(self) -> object:
        """Test test pd extension is platform specific."""
    ...
    def test_pd_build_instructions(self) -> object:
        """Test test pd build instructions."""
        platform = PureDataPlatform()
        instructions = platform.get_build_instructions()
...
class TestMaxPlatform:
    ...
    def test_max_extension_is_platform_specific(self) -> object:
        """Test test max extension is platform specific."""
        platform = MaxPlatform()
        ext = platform.extension
        assert ext in [".mxo", ".mxe64", ".mxl"]

    def test_max_build_instructions(self) -> object:
        """Test test max build instructions."""
        platform = MaxPlatform()
        instructions = platform.get_build_instructions()
...
class TestCrossPlatformGeneration:
    ...
    def test_generate_project_creates_directory(
        self, platform_key: str, gigaverb_export: Path, tmp_path: Path
    ) -> object:
    ...
    def test_generate_project_copies_gen_export(
        ...
        assert len(gen_files) > NUM_0, f"{platform_key}: no .cpp files in gen/"
    ...
    def test_generate_project_has_build_system_file(
        ...
        project_dir = _generate_project(platform_key, gigaverb_export, tmp_path)
        build_file = BUILD_SYSTEM_FILES.get(platform_key)
    ...
    def test_generate_project_with_buffers(
        self, platform_key: str, rampleplayer_export: Path, tmp_path: Path
    ) -> object:
